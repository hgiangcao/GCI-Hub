...
<body class="bg-dark-bg text-dark-text font-sans min-h-screen flex flex-col antialiased">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-14">
                <div class="flex items-center gap-8">
                    <a href="problemset.php" class="flex-shrink-0 cursor-pointer">
                        <span class="text-brand-orange font-bold text-xl tracking-tight">&lt;/&gt; GCIOJ</span>
                    </a>
                    <div class="hidden md:block">
                        <div class="flex items-baseline space-x-4">
                            <a href="problemset.php" class="text-dark-text bg-dark-hover px-3 py-2 rounded-md text-sm font-medium transition">Problems</a>
                            <a href="#" class="text-dark-muted hover:text-dark-text hover:bg-dark-hover px-3 py-2 rounded-md text-sm font-medium transition">Contest & Ranking</a>
                        </div>
                    </div>
                </div>
                <div class="flex items-center gap-4">
                    <?php if(isset($_SESSION['user_id'])): ?>
                        <span class="text-sm text-dark-muted">Welcome, <?= htmlspecialchars($_SESSION['student_id']) ?></span>
                ...
                <tbody class="divide-y divide-gray-700 text-sm">
                    <?php if (count($problems) > 0): ?>
                        <?php foreach ($problems as $row): ?>
                            <tr class="hover:bg-dark-hover transition group cursor-pointer" onclick="window.location='problem.php?id=<?= $row['id'] ?>'">
    ...
    <footer class="bg-dark-surface border-t border-gray-700 py-8 mt-auto">
        <div class="max-w-7xl mx-auto px-4 text-center text-dark-muted text-sm">
            <p>&copy; 2025 GCIOJ. All rights reserved.</p>
        </div>
    </footer>
