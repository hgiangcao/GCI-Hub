...
def setup_test_data():
    global paper_score, coding_score
    paper_score = random.randint(0, 100)
    coding_score = random.randint(0, 100)
    return [paper_score, coding_score]

def teacher_solve():
    # The Correct Logic: Max of integer a and b
    final_score = paper_score * 0.7 + coding_score*0.3
...
def auto_grade():
    ...
    for i in range(1, n_test + 1):
        ...
        if is_close_value(expected_str,last_line):
            ...
            wa_mess += (f"   Input: paper_score = {inputs[0]}; coding_score = {inputs[1]}\n")
