        ...
        if (empty($rawTitle)) {
            ...
            try {
                Problem::create(
                    ...
                    (int)$_POST['time_limit_ms'], (int)$_POST['memory_limit_mb'], $_POST['tag'], $_POST['forbidden_keyword']
                );
...
<body class="bg-dark-bg text-dark-text font-sans min-h-screen p-6">
        ...
        <form method="POST" id="problemForm" class="grid grid-cols-1 xl:grid-cols-12 gap-6 h-full">
            ...
            <div class="xl:col-span-3 space-y-5">
                ...
                <div class="bg-dark-surface p-4 rounded border border-dark-border">
                    ...
                    <div class="space-y-3">
                        <div>
    <label class="text-xs text-dark-muted block mb-1">Forbidden Keywords (e.g. for, while)</label>
    <input type="text" name="forbidden_keyword" id="inp_forbidden" value="" placeholder="for,while,import" class="w-full bg-dark-bg border border-dark-border rounded px-3 py-2 text-sm font-mono focus:border-brand-orange outline-none">
</div>
                        <div class="grid grid-cols-2 gap-3">
                            <div>
    ...
    <script>
        ...
        function loadProblem(id, updateUrl = false) {
            const inputs = ['inp_title', 'inp_level', 'inp_tag', 'inp_time', 'inp_mem', 'inp_input', 'inp_output', 'inp_grading', 'inp_lid', 'inp_llink', 'inp_forbidden'];
            ...
            fetch('?action=fetch&id=' + id)
                ...
                .then(res => {
                    if(res.status === 'success') {
                        ...
                        document.getElementById('inp_forbidden').value = d.forbidden_keyword || ""; // Populate from DB
