...
class Contest
{
    ...
    public static function create($name, $course, $start, $end, $isActive, $isPublic, $antiCheat = 0)
    {
        $sql = "INSERT INTO contest (name, course, start_time, end_time, is_active, is_public, anti_cheat) VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = DB::connect()->prepare($sql);
        return $stmt->execute([$name, $course, $start, $end, $isActive, $isPublic, $antiCheat]);
    }

    // Update (Updated with flags)
    public static function update($id, $name, $course, $start, $end, $isActive, $isPublic, $antiCheat = 0)
    {
        $sql = "UPDATE contest SET name=?, course=?, start_time=?, end_time=?, is_active=?, is_public=?, anti_cheat=? WHERE id=?";
        $stmt = DB::connect()->prepare($sql);
        return $stmt->execute([$name, $course, $start, $end, $isActive, $isPublic, $antiCheat, $id]);
    }

    // ... (Keep getAll, getById, delete, addProblem as they were before) ...    public static function getAll()
